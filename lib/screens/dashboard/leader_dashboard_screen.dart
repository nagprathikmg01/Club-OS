...
class LeaderDashboardScreen extends StatelessWidget {
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('NETWORK ACCESS KEY', style: TextStyle(color: ClubOsTheme.onSurfaceVariant, fontSize: 9, letterSpacing: 2, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 8),
                        Text(
                          dataProvider.activeClub.joinCode.isEmpty ? 'PENDING...' : dataProvider.activeClub.joinCode.toUpperCase(),
                          style: TextStyle(color: ClubOsTheme.primaryCommand, fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: 6),
                        ),
                      ],
                    ),
                    IconButton(
                      icon: Icon(Icons.copy_all, color: ClubOsTheme.primaryCommand),
                      onPressed: () {
                        final code = dataProvider.activeClub.joinCode;
                        if (code.isNotEmpty) {
                          Clipboard.setData(ClipboardData(text: code));
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('KEY COPIED TO CLIPBOARD'), backgroundColor: ClubOsTheme.primaryCommand),
                          );
                        }
                      },
                    ),
                        ...
                        child: Row(
                          children: [
                            CircleAvatar(
                              ...
                              child: Text(request.userName[0].toUpperCase(), style: TextStyle(color: ClubOsTheme.primaryCommand)),
                            ),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    request.userName,
                                    style: TextStyle(color: ClubOsTheme.onSurfaceMain, fontWeight: FontWeight.bold),
                                    overflow: TextOverflow.ellipsis,
                                    maxLines: 1,
                                  ),
                                  Text(
                                    request.userEmail.toLowerCase(),
                                    style: TextStyle(color: ClubOsTheme.onSurfaceVariant, fontSize: 12),
                                    overflow: TextOverflow.ellipsis,
                                    maxLines: 1,
                                  ),
                                  const SizedBox(height: 4),
                                  Text('REQUESTED: ${request.requestedAt.hour}:${request.requestedAt.minute.toString().padLeft(2, '0')}', style: TextStyle(fontSize: 9, color: ClubOsTheme.tertiaryAnalytical, fontWeight: FontWeight.bold)),
                                ],
                              ),
                            ),
                            ElevatedButton(
  ...
  void _showRoleSelectionDialog(BuildContext context, DataProvider provider, String userId, String userName) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('ASSIGN CLEARANCE ROLE', style: Theme.of(context).textTheme.labelSmall),
        content: Column(
  ...
  Widget _buildRoleOption(BuildContext context, DataProvider provider, String userId, String role, String description) {
    return ListTile(
      ...
      title: Text(role, style: TextStyle(fontWeight: FontWeight.bold, color: ClubOsTheme.primaryCommand, fontSize: 13, letterSpacing: 1)),
      subtitle: Text(description, style: TextStyle(fontSize: 10, color: ClubOsTheme.onSurfaceVariant)),
      trailing: Icon(Icons.arrow_forward_ios, size: 12, color: ClubOsTheme.primaryCommand),
      onTap: () {
    ...
    return _buildBentoCard(
      ...
      child: Column(
        ...
        children: [
           ...
           const SizedBox(height: 4),
           Text(label, style: TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: ClubOsTheme.onSurfaceVariant, letterSpacing: 1)),
        ],
      ),
    );
  }
